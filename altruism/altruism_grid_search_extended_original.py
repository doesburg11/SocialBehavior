...
def main():
    ...
    grid = {
        ...
        'disease': [0.25],
        'harshness': [0.85, 0.86, 0.87, 0.88, 0.89, 0.90, 0.91, 0.92, 0.93, 0.94, 0.95, 0.96, 0.97, 0.98, 0.99, 1.00],
        'altruistic_probability': [0.39],   # <-- Added
        'selfish_probability': [0.39]       # <-- Added
    }
    ...
    for combo in param_combos:
        ...
        row = {
            ...
            'altruist_avg': float(altruist_avg),   # <-- Averaged over replicates
            'selfish_avg': float(selfish_avg),     # <-- Averaged over replicates
            'black_avg': float(black_avg)          # <-- Averaged over replicates
        }
        import csv
