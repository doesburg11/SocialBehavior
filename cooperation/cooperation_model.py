...
class Cow:
    ...
    def reproduce(self, cows):
        if self.energy > reproduction_threshold:
            ...
            # New cow at same location, same breed
            cows.append(Cow(self.x, self.y, self.breed, metabolism * 4))
        ...
        for x in range(grid_size):
            for y in range(grid_size):
                if self.grass[x, y] >= low_high_threshold:
                    if random.uniform(0, 100) < high_growth_chance:
                        self.grass[x, y] += 1
                else:
                    if random.uniform(0, 100) < low_growth_chance:
                        self.grass[x, y] += 1
